...
const StockAppV35 = () => {
  ...
  const handlePinSubmit = (e) => {
    ...
    if (pinInput === '4510' && selectedRole) {
      setCurrentUser({
        name: selectedRole === 'owner' ? 'เจ้าของ' : (selectedRole === 'backoffice' ? 'ทีมหลังบ้าน' : 'ทีมหน้าร้าน'),
        role: selectedRole,
      });
      setPinInput('');
        ...
        <div style={{
          ...
          {!selectedRole ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {['owner', 'backoffice', 'frontstore'].map((role) => (
                <button key={role}
                  ...
                  {role === 'owner' ? text.owner : (role === 'backoffice' ? text.backoffice : text.frontstore)}
                </button>
              ))}
            </div>
          ) : (
